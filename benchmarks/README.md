...
### Analyze MoE FFN memory snapshots

The FFN chunk memory debug path can dump allocator history snapshots around the
routed expert MLP. Analyze one snapshot or a directory containing matching
chunk OFF/ON runs with:

```shell
python benchmarks/scripts/analyze_ffn_memory_snapshot.py \
    /tmp/moe_ffn_snapshot \
    --json /tmp/moe_ffn_memory_report.json \
    --csv /tmp/moe_ffn_memory_comparison.csv \
    --html-dir /tmp/moe_ffn_memory_html
```

The text report includes reconstructed allocated, active, and reserved memory
peaks; peak growth from the beginning of the routed FFN; allocator
fragmentation; event and stream counts; allocations live at the peak; and
allocation call-site hotspots. Snapshot names generated by the FFN debug path
are automatically paired by global rank and routed-token count for chunk OFF/ON
comparison.

Useful options:

- `--brief`: print only the OFF/ON comparison.
- `--device all`: analyze every allocator device recorded in each pickle.
- `--top N`: control the number of allocation and call-site rows.
- `--recursive`: recursively search input directories for pickle files.

The generated HTML files embed the snapshot and load the PyTorch MemoryViz
frontend from jsDelivr when opened. Use the numeric `allocated peak` and `FFN
peak growth` fields for comparison; use the HTML timeline to identify which
allocations create the peak.
